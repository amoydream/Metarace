...
class sprnd(object):
    ...
    def add_ucipenalty(self, rlist):
        ...
        fine = ' '
        try:
            bib,code,amount = rlist.split();
            c = code[0]
            ucicode = c.upper()
            ucipen = code[1:]
        except (ValueError):
            bib,code = rlist.split();
            c = code[0]
            ucicode = c.upper()
            ucipen = code[1:]
        name = self.rider_name(bib)  
        ucicodedes = strops.get_ucicode(ucicode)
        ucipendes = strops.get_ucipen(ucipen)
        if ucicodedes == None:
            self.log.error('Invalid UCI Code entered.')
            return
        if ucipendes == None:
            self.log.error('Invalid UCI Code entered.')
            return
        if ucicode != 'B':
            if amount != '':
                self.log.error('Invalid action, Fine amount not required for this Penalty Code.')
                return
        if ucicode == 'B':
            if amount != '':
                fine = (u' $'+ amount +' ')
            elif amount == '':
                self.log.error('Invalid action, Fine amount required for this Penalty Code.')
                return
        comment = (u'Rider ' + bib + ', ' + name + '. ' + ucicode + ucipen + ' '+ unicode(ucicodedes) + fine + unicode(ucipendes))
        print comment
    ...
    def loadconfig(self):
        ...
        if self.evtype in ['sprint', 'sprint round','sprint final', 'keirin']:
            deftimetype = '200m'
            defdistunits = 'metres'
            defdistance = '200'
        ...
        self.onestart = False
        self.set_elapsed()
        if self.evtype in ['sprint', 'keirin','sprint round','sprint final']:
            deftimetype = '200m'
            defdistunits = 'metres'
            defdistance = '200'
        if self.autospec:
    ...
    def sprnd_ctrl_action_activate_cb(self, entry, data=None):
        ...
        if acode == 'add':
            ...
            entry.set_text('')
        #elif acode == 'uci':
        #    self.add_ucipenalty(rlist)
        #    entry.set_text('')
        elif acode == 'con':
            for cv in rlist.split():
                self.add_contest(cv)
            entry.set_text('')
            return True
        else:
    ...
    def saveconfig(self):
        ...
        cw.set(u'event', u'round', self.round)
        contestlist = [] 
        for c in self.contests:
    ...
    def sprnd_ctrl_winner_activate_cb(self, entry, data=None):
    ...
    def sprnd_info_time_edit_activate_cb(self, button):
	...
	self.timetype = 'Start/finish'
        ...
        # ctrl pane
        self.stat_but = b.get_object('sprnd_ctrl_stat_but')
        self.ctrl_winner = b.get_object('sprnd_ctrl_winner')
        self.ctrl_action_combo = b.get_object('sprnd_ctrl_action_combo')
        self.ctrl_action = b.get_object('sprnd_ctrl_action')
        self.action_model = b.get_object('sprnd_action_model')
        #self.ctrl_places = b.get_object('sprnd_ctrl_places')
        ...
        t.show()
        b.get_object('sprnd_result_win').add(t)
