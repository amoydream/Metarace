...
CSV_REPORT_DEFAULT_COLUMNS = [
	u'type', u'head', u'subh', u'foot', u'unit', u'unit1', u'unit2', u'colu', u'sour'
]
...
def vec2htmlrow(vec=[]):
    ...
    cols.append(htlib.td(htlib.escapetext(rowmap[5]),
                           {u'class':u'right'}))	# time/gap (right)
    cols.append(htlib.td(htlib.escapetext(rowmap[6]),
                           {u'class':u'right'}))	# time/gap (right)
    cols.append(htlib.td(htlib.escapetext(rowmap[7]),
                           {u'class':u'right'}))	# time/gap (right)
    cols.append(htlib.td(htlib.escapetext(rowmap[8])))	# Units (left)
    return htlib.tr(cols)
...
def vec2htmlhead(vec=[]):
    ...
    cols.append(htlib.th(htlib.escapetext(rowmap[5]),
                           {u'class':u'right'}))	# time/gap (right)
    cols.append(htlib.th(htlib.escapetext(rowmap[6]),
                           {u'class':u'right'}))	# time/gap (right)
    cols.append(htlib.th(htlib.escapetext(rowmap[7]),
                           {u'class':u'right'}))	# time/gap (right)
    cols.append(htlib.th(htlib.escapetext(rowmap[8])))	# Units (left)
    return htlib.tr(cols)
    ...
    def draw_xls(self, report, worksheet):
        ...
        if len(self.lines) > 0:
            ...
            for rw in rows:
                ...
                worksheet.write(row, 6, l[6], XS_RIGHT)
                worksheet.write(row, 7, l[7], XS_RIGHT)
                worksheet.write(row, 8, l[8], XS_LEFT)
                row += 1
            row += 1
        if self.footer:
    ...
    def draw_xls(self, report, worksheet):
        ...
        if len(self.lines) > 0:
            ...
            for l in rows:
                ...
                worksheet.write(row, 4, l[4], XS_RIGHT)
                worksheet.write(row, 5, l[5], XS_RIGHT)
                worksheet.write(row, 5, l[6], XS_RIGHT)
                worksheet.write(row, 5, l[7], XS_RIGHT)
                worksheet.write(row, 6, l[8], XS_LEFT)
                row += 1
            row += 1
        if self.footer:
            worksheet.write(row, 2, self.footer.strip(), XS_SUBTITLE)
...
class rttstartlist(object):
    ...
    def draw_xls(self, report, worksheet):
        ...
        if len(self.lines) > 0:
            rows = []
            if self.colheader:
                rows.append(vecmapstr(self.colheader,9))
            for r in self.lines:
                nv = r[0:6]
                if len(nv) == 2:
                    nv = [nv[0], None, nv[1]]
...
class section(object):
    ...
    def get_h(self, report):
        ...
        if self.h is None or len(self.lines) != self.lcount:
            self.lcount = len(self.lines)
            for l in self.lines:
                if len(l) > 8 and l[8] and type(l[8]) is list:
                    self.lcount+= 1
            self.h = report.line_height * self.lcount
            if self.colheader:	# colheader is written out with body
                self.h += report.line_height
    ...
    def draw_pdf(self, report):
        ...
        if len(self.lines) > 0:
            ...
            for r in self.lines:
                ...
                if len(r) > 8 and type(r[8]) is list:
                    report.h += report.standard_row(report.h,r[8],grey)
            #eh = report.h	- for the column shade box
