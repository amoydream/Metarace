...
class ps(object):
    ...
    def loadconfig(self):
        ...
        self.altlappoints = strops.confopt_posint(
                                cr.get('event','altlappoints'),10)
        ...
        if self.inomnium or len(self.sprintsource) > 0:
            for s in self.sprints:
                sid = s[SPRINT_COL_ID]
...
# what is the SID
    ...
    def saveconfig(self):
        ...
        cw.set('event', 'altlappoints', self.altlappoints)
    ...
    def result_report(self, recurse=False):
        ...
        if len(self.comments) > 0:
            ...
            templist = self.comments
            for temp in templist.split("!"):
                sec.lines.append([None, temp])
            ret.append(sec)
    ...
    def startlist_report(self, program=False):
        ...
        if self.evtype == 'madison' or u't' in self.series:
                    ...
                    if rh is not None:
                        ...
                        if len (tvec) == 2:
                            ...
                            if trh is not None:
                                trname = trh[u'namestr']
                                trinf = trh[u'ucicode']
                            sec.lines.append([None, None, trname, trinf,
                                                None, u'Red', None,])
                            trname = u''
                            trinf = u''
    ...
    def standingstr(self, width=None):
        ...
        cur = 1
        enn = 1		# count 'n'
        ofemm = 0	# of 'm' lap numbered sprints
        for s in self.sprints:
    ...
    def ps_ctrl_places_activate_cb(self, entry, data=None):
        ...
        if self.checkplaces(places):
            ...
            if prevplaces == '':
                FMT = [(2, u'l'), (3, u'r'), u' ',
                  (self.meet.scb.linelen-9, u'l'), u' ', (2, u'r')]
                self.meet.scbwin = scbwin.scbintsprint(self.meet.scb,
                                   self.meet.racenamecat(self.event),
                                   sinfo.upper(),
    ...
    def ps_ctrl_action_activate_cb(self, entry, data=None):
        ...
        if acode == 'gain':
            ...
            self.add_comments(rlist)
            entry.set_text('')
        else:
            self.log.error('Ignoring invalid action.')
        glib.idle_add(self.delayed_announce)

    def add_comments(self, newcomments):
        """Append a race comment."""
        if len(self.comments)>0:
            self.comments += (u'!' + newcomments)
        elif len(self.comments) == 0:
            self.comments += newcomments
        self.log.info('Added race comment: ' + repr(newcomments))
        self.saveconfig()
...
#            points = [5, 3, 2, 1]	# Default is four places
    ...
    def reset_lappoints(self):
        ...
        if self.masterslaps:
            self.lappoints = self.altlappoints
        else:
            self.lappoints = 20
    ...
    def __init__(self, meet, event, ui=True):
        ...
        self.lappoints = 20
        self.altlappoints = 10	# NJTS change
        self.scoring = 'points'
        self.distance = None
        self.units = 'laps'
        self.sprintlaps = ''
