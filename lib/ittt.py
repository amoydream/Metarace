...
class ittt(object):
    ...
    def todstr(self, col, cr, model, iter, data=None):
        ...
        if ft is not None:
            ...
            rt = model.get_value(iter, COL_RT)
            ql = model.get_value(iter, COL_QL)
            hl = model.get_value(iter, COL_HL)
            if st is None:
                st = tod.tod(0)
            if st == tod.tod(0):
    ...
    def loadconfig(self):
        ...
        defchan200 = str(timy.CHAN_200)
        defchan100 = str(timy.CHAN_100)
        defautoarm = 'No'
        ...
        self.chan_200 = strops.confopt_chan(cr.get('event', 'chan_200'), defchan200)
        self.chan_100 = strops.confopt_chan(cr.get('event', 'chan_100'), defchan100)
        ...
        self.qlreport = strops.confopt_bool(cr.get('event', 'qlreport'))
        self.hlreport = strops.confopt_bool(cr.get('event', 'hlreport'))
        ...
        for r in cr.get('event', 'startlist').split():
            ...
            if cr.has_option('riders', r):
                ...
                if len(ril) >= 6:	# Reaction ToD
                    rt = tod.str2tod(ril[5])
                if len(ril) >= 7:	# Quarter Lap ToD
                    ql = tod.str2tod(ril[6])
                if len(ril) >= 8:	# Half ToD
                    hl = tod.str2tod(ril[7])
                j = 8
            ...
            self.settimes(nri, st, ft, rt, ql, hl, sp, doplaces=False, comment=co)
    ...
    def saveconfig(self):
        ...
        cw.set('event', 'qlreport', self.qlreport)
        cw.set('event', 'hlreport', self.hlreport)
        ...
        for r in self.riders:
            ...
            tl = [r[COL_START], r[COL_FINISH], r[COL_RT], r[COL_QL], r[COL_HL]]
    ...
    def result_report(self, recurse=False):
        ...
        for r in self.riders:
            ...
            sec.lines.append([rank, rno, rname, None, rtime, dtime,None])
            ...
            if u't' in self.series:
                for trno in strops.reformat_riderlist(rh[u'note']).split():
                    ...
                    if trh is not None:
                        ...
                        sec.lines.append([None, trno, trname, None,
                                                None, None, None])
    ...
    def analysis_report(self, recurse=False):
        ...
        for r in self.riders:
            ...
            if self.onestart:
                ...
                if r[COL_COMMENT]:
                    rtime = str(r[COL_COMMENT])
                if r[COL_QL] is not None:
                    qlap = ((r[COL_QL]-r[COL_START]).truncate(3).ref2str().strip())
                    if self.teamnames: 
                        qlrank = u'(' + str(self.qlsplits.rank(rnot)+1) + u')'
                    else:
                        qlrank = u'(' + str(self.qlsplits.rank(rno)+1) + u')'
                else:
                    qlrpt = u' '
                    qlrank = u' '
                if r[COL_HL] is not None:
                    if r[COL_QL] is not None:
                        hlap = ((r[COL_HL]-r[COL_QL]).truncate(3).ref2str().strip())
                    hlrpt = (r[COL_HL]-r[COL_START]).truncate(3).ref2str().strip()
                    if self.teamnames:             
                        hlrank = u'(' + str(self.hlsplits.rank(rnot)+1) + u')'
                    else:
                        hlrank = u'(' + str(self.hlsplits.rank(rno)+1) + u')'
                else:
                    hlap = u' '
                    hlrpt = u' '
                    hlrank = u' '
            seca.lines.append([rank, rno, rname, None, rtime, dtime,None])
            ...
            if u't' in self.series:
                for trno in strops.reformat_riderlist(rh[u'note']).split():
                    ...
                    if trh is not None:
                        trname = trh[u'namestr']
                        trinf = trh[u'ucicode']
                        seca.lines.append([None, trno, trname, trinf,
                                                None, None, None])
            ...
            if rh is not None:
                ...
                if rh[u'cat'] and u'tandem' in rh[u'cat'].lower():
                    ...
                    if ph is not None:
                        prname = ph[u'namestr'] + u' - Pilot'
                        prinf = ph[u'ucicode']
                        seca.lines.append([None, u'', prname, prinf,
                                                None, None, None])
            if self.qlreport:
                seca.lines.append([None, u'Qrt Lap', qlap, qlrpt, qlrank, None, None])
            if self.hlreport:
                seca.lines.append([None, u'Half Lap', hlap, hlrpt,hlrank, None, None])
            ...
            if r[COL_PLACE] != '':
                if self.teamnames:
                    if cr.has_option('riders', rnot):
                        ril = csv.reader([cr.get('riders', rnot)]).next()
                        j = 8
                        k = 1
                        priorlap = None
                        while j < len(ril):	# Split ToDs
                            ...
                            if spt is None:
                                ...
                                if k ==1:
                                    lap = ((spt-r[COL_START]).truncate(3).ref2str().strip())
                                    laptotal = ((spt -r[COL_START]).truncate(3).ref2str().strip())
                                    laprank = u'(' + str(self.splits[k-1].rank(rnot)+1) + u')'
                                    seca.lines.append([None, u'Lap 1', lap, laptotal, laprank, None, None])
                                    priorlap = spt
                                else:
                                    lap = ((spt - priorlap).truncate(3).ref2str().strip())
                                    laptotal = ((spt -r[COL_START]).truncate(3).ref2str().strip())
                                    laplable = u'Lap ' + str(k)
                                    laprank = u'(' + str(self.splits[k-1].rank(rnot)+1) + u')'
                                    priorlap = spt
                                    seca.lines.append([None, laplable, lap, laptotal, laprank, None, None])
                                j += 1
                                k += 1
                        if priorlap is None:
                            lap = u' '
                        else:
                            lap = ((r[COL_FINISH]-priorlap).truncate(3).ref2str().strip())
                        laptotal = ((r[COL_FINISH]-r[COL_START]).truncate(3).ref2str().strip())
                        laplable = u'Lap ' + str(k)
                        laprank = u'(' + r[COL_PLACE] + u')'
                        seca.lines.append([None, laplable, lap, laptotal, laprank, None, None])
                else:
                    if cr.has_option('riders', rno):
                        ril = csv.reader([cr.get('riders', rno)]).next()
                        j = 8
                        k = 1
                        priorlap = None
                        while j < len(ril):	# Split ToDs
                            ...
                            if spt is None:
                                ...
                                if k ==1:
                                    lap = ((spt-r[COL_START]).truncate(3).ref2str().strip())
                                    laptotal = ((spt -r[COL_START]).truncate(3).ref2str().strip())
                                    laprank = u'(' + str(self.splits[k-1].rank(rno)+1) + u')'
                                    seca.lines.append([None, u'Lap 1', lap, laptotal, laprank, None, None])
                                    priorlap = spt
                                else:
                                    lap = ((spt - priorlap).truncate(3).ref2str().strip())
        ...
        if self.onestart:
            ...
            if rcount > 0 and pcount < rcount:
                seca.subheading += u' - STANDINGS'
            else:
                seca.subheading += u' - Result'
        else:
            if substr:
                seca.subheading = substr
        ...
        # comment
        if self.comment is not None:
            seca = printing.bullet_text()
            seca.heading = u'Decisions of the commissaires panel'
            seca.lines.append([None, self.comment])
            reta.append(sec)
        ...
        if self.difftime:
            if self.diffstart is None or self.difflane is sp:
                ...
                if so.lap == sp.lap and self.diffstart is not None:
                    ...
                    self.difflane = None
                    self.diffstart = None
        if self.timerwin and type(self.meet.scbwin) is scbwin.scbtt or scbwin.scbttvw:
    ...
    def qllap_trig(self, sp, t):
        """Register Quater lap trigger."""
        bib = sp.getrider()
        self.qlsplits.insert(t-self.curstart, bib)
        rank = self.qlsplits.rank(bib)
        #self.log_qlsplit(sp.getrider(), self.curstart, t)
        sp.intermedsr(t, 1)
        if sp == self.fs:
            self.fsqlsplit = t
        if sp == self.bs:
            self.bsqlsplit = t - self.delay
        self.meet.timer.arm(self.chan_B)
        self.meet.timer.arm(self.chan_A)

    def hllap_trig(self, sp, t):
        """Register Half lap trigger."""
        bib = sp.getrider()
        self.hlsplits.insert(t-self.curstart, bib)
        rank = self.hlsplits.rank(bib)
        #self.log_hlsplit(sp.getrider(), self.curstart, t)
        sp.intermedsr(t, 1)
        if sp == self.fs:
            self.fshlsplit = t
        if sp == self.bs:
            self.bshlsplit = t
        self.meet.timer.dearm(self.chan_200)
        self.meet.timer.dearm(self.chan_100)
    ...
    def fin_trig(self, sp, t):
        ...
        if ri is not None:
            self.settimes(ri, self.curstart, t, rt, ql, hl, sp.splits)
        else:
    ...
    def timercb(self, e):
        ...
        if self.timerstat == 'armstart':
            ...
            if chan == self.chan_B:
                ...
                if stat == 'running':
                    self.hllap_trig(self.fs, e)
            if chan == self.chan_200:
                stat = self.fs.getstatus()
                if stat == 'running':
                    self.qllap_trig(self.fs, e)
            if chan == self.chan_100:
                stat = self.bs.getstatus()
                if stat == 'running':
                    self.qllap_trig(self.bs, e)
        return False
        ...
        if self.timetype == 'single':
            ...
            if self.timerwin and type(self.meet.scbwin) is scbwin.scbtt or scbwin.scbttvw:
                glib.timeout_add_seconds(3, self.show_200_ttb,
                        self.meet.scbwin)
        self.meet.timer.arm(self.chan_200)
        self.meet.timer.arm(self.chan_100)
    ...
    def addrider(self, bib='', info=None):
        ...
        if info is not None:
            istr = str(info)
        nr=[bib, '', '', '', '', istr, '', None, None, None, None, None, None]
    ...
    def settimes(self, iter, st=None, ft=None, rt=None, ql=None, hl=None, splits=None,
        ...
        self.qlsplits.remove(bib)
        self.hlsplits.remove(bib)
        # assign tods
        self.riders.set_value(iter, COL_START, st)
        self.riders.set_value(iter, COL_FINISH, ft)
        self.riders.set_value(iter, COL_RT, rt)
        self.riders.set_value(iter, COL_QL, ql)
        self.riders.set_value(iter, COL_HL, hl)
        # save result
        if st is None:
            st = tod.tod(0)
        if ft is not None:
            if ql is not None:
                self.qlsplits.insert(ql - st, bib)
            if hl is not None:
                self.hlsplits.insert(hl - st, bib)
    ...
    def fmtridername(self, tp):
        ...
        if bib != '':
            ...
            if r is not None and r[COL_BIB] != '':
                if self.teamnames:
                    club = r[COL_CLUB].decode('utf-8')
                    name = u'  ' + r[COL_FIRSTNAME].decode('utf-8')
    ...
    def fsteamriders(self, tp):
        rno = tp.getrider().strip()
        triderno1 = u' '
        trider1 = u' '
        trider1s = u' '
        triderno2 = u' '
        ...
        if rno != '':
            ...
            for trno in strops.reformat_riderlist(rh[u'note']).split():
                ...
                if trh is not None:
                    if i == 0:
                        triderno1 = trh[u'bib'] + u'. '
                        trider1 = trh[u'namestr']
                        trider1s = trh[u'first'][0] + u'.' + trh[u'last']
                    if i == 1:
                        triderno2 = trh[u'bib'] + u'. '
                        trider2 = trh[u'namestr']
                        trider2s = trh[u'first'][0] + u'.' + trh[u'last']
                    if i == 2:
                        triderno3 = trh[u'bib'] + u'. '
                        trider3 = trh[u'first'][0] + u'.' + trh[u'last']
                    if i == 3:
                        triderno4 = trh[u'bib'] + u'. '
                        trider4 = trh[u'first'][0] + u'.' + trh[u'last']
                    i += 1
            if i < 3:
                self.fsteamline1 = triderno1 + trider1 
                self.fsteamline2 = triderno2 + trider2
            else:
                self.fsteamline1 = '{:19} {:19}'.format(str(triderno1 + trider1s),str(triderno3 + trider3))
                self.fsteamline2 = '{:19} {:19}'.format(str(triderno2 + trider2s),str(triderno4 + trider4))
        else:
            self.fsteamline1 = ''
            self.fsteamline2 = ''

                
    def bsteamriders(self, tp):
        rno = tp.getrider().strip()
        triderno1 = u' '
        trider1 = u' '
        trider1s = u' '
        ...
        if rno != '':
            ...
            for trno in strops.reformat_riderlist(rh[u'note']).split():
                ...
                if trh is not None:
                    ...
                    i += 1
            if i < 3:
                self.bsteamline1 = triderno1 + trider1 
                self.bsteamline2 = triderno2 + trider2 
            else:
                self.bsteamline1 = '{:19} {:19}'.format(str(triderno1 + trider1s),str(triderno3 + trider3))
                self.bsteamline2 = '{:19} {:19}'.format(str(triderno2 + trider2s),str(triderno4 + trider4))
                '{:10}'.format('test')
        else:
            self.bsteamline1 = ''
            self.bsteamline2 = ''
    ...
    def showtimerwin(self):
        ...
        self.meet.scbwin = None
        if self.teamnames and self.meet.videowall:
            self.fsteamriders(self.fs)
            self.bsteamriders(self.bs)
            self.meet.scbwin = scbwin.scbttvw(self.meet.scb,
                                    self.meet.racenamecat(self.event),
                                    self.fmtridername(self.fs), self.fsteamline1 , self.fsteamline2,
                                    self.fmtridername(self.bs), self.bsteamline1 , self.bsteamline2)
        else:
        ...
        self.timerwin = True
        if self.meet.videowall:
            self.meet.scbwin.redraw()
        else:
            self.meet.scbwin.reset()
    ...
    def log_hlsplit(self, bib, start, split, prev=None):
        """Print lap split log."""
        if prev is None:
            prev = start
        self.meet.timer_log_straight(bib, split-prev, 3)
        if lap > 1 and prev != start:
            self.meet.timer_log_straight(bib, 'time', split - start, 3)
            
    def log_qlsplit(self, bib, start, split, prev=None):
        """Print lap split log."""
        if prev is None:
            prev = start
        self.meet.timer_log_straight(bib, split-prev, 3)
    ...
    def __init__(self, meet, event, ui=True):
        ...
        self.chan_200 = timy.CHAN_200
        self.chan_100 = timy.CHAN_100
        self.fsvec = None
        self.bsvec = None
        self.fslog = None
        self.bslog = None
	...
	self.splits = []		# list of lap split lists
        self.qlsplits = tod.todlist('QL')
        self.hlsplits = tod.todlist('HL')
        self.results = tod.todlist('FIN')
        self.fsqlsplit = None
        self.bsqlsplit = None
        self.fshlsplit = None
        self.bshlsplit = None
        self.fsteamline1 = None
        self.fsteamline2 = None
        self.bsteamline1 = None
        self.bsteamline2 = None
        ...
        self.riders = gtk.ListStore(gobject.TYPE_STRING,   # 0 bib
                                    ...
                                    gobject.TYPE_PYOBJECT, # 9 Reaction Time
                                    gobject.TYPE_PYOBJECT, # 10 Quarter Lap
                                    gobject.TYPE_PYOBJECT, # 11 Half Lap
                                    gobject.TYPE_PYOBJECT) # 12 Lap Splits(0,n)
