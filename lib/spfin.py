...
class spfin(object):
    ...
    def do_places(self, contest, winno, winner, winpl,
        ...
        if contest == 'Heat 1':
            ...
            for cr in self.contests:
                if cr[COL_CONTEST] == 'Heat 1':
                    if cr[COL_WINNER] == winno:
                        aw = '**'
                        bw = ' '
                    elif cr[COL_WINNER] != winno:
                        aw = '*'
                        bw = '*'
        elif contest == 'Heat 3':
            aw = '**'
            bw = '*'
        startlist = [ [u'1.', winno, winner, aw],
                      [u'2.', loseno, loser,bw]]
        if ftime:
            startlist.append([u'', u'', u''])
            startlist.append([u'', u'', u'200m: ' + ftime])
        name_w = self.meet.scb.linelen- 10
        fmt = [(3,'l'),(4,'r'),u' ',(name_w,'l'),(2,'r')]
    ...
    def do_startlist(self):
        ...
        if i is not None:	# contest selected ok
            ...
            self.meet.announce.gfx_add_row([an, ar, '', aw])
            startlist.append([an, ar, aw])
            bn = self.contests.get_value(i, COL_B_NO)
            br = self.contests.get_value(i, COL_B_STR).decode('utf-8')
            self.meet.announce.gfx_add_row([bn, br, '',bw])
            startlist.append([bn, br, bw])
        self.meet.scbwin = None
        self.timerwin = False
        name_w = self.meet.scb.linelen-7
        fmt = [(4,'r'),u' ',(name_w,'l'),(2,'r')]
        self.meet.scbwin = scbwin.scbintsprint(self.meet.scb,
                                   self.meet.racenamecat(self.event),
                                   cid, fmt, startlist)
        self.meet.scbwin.reset()
    ...
    def spfin_ctrl_winner_activate_cb(self, entry, data=None):
    ...
    def result_gen(self):
        ...
        hw2 = ''
        for c in self.contests:
            if c[COL_CONTEST] == 'Heat 1':
                hw1 = c[COL_WINNER]
            if c[COL_CONTEST] == 'Heat 2':
                hw2 = c[COL_WINNER]
                if hw1 == hw2:
                    rank = None
                    time = None
